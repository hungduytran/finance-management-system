...
interface DebtAddDialogProps {
  onSuccess: () => void;
}

const DebtAddDialog = ({ onSuccess }: DebtAddDialogProps) => {
  ...
  const addDebtMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success("Thêm nợ mới hoàn tất", {
        description: "Bạn đã thêm nợ mới thành công",
        duration: 3000,
      });
      onSuccess();
    ...
    onError: (error) => {
      toast.error("Thêm nợ mới thất bại", {
  ...
  return (
    <Fragment>
      ...
      >
        <DialogTitle>Nợ mới</DialogTitle>
        <DialogContent>
